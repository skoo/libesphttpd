...
int ICACHE_FLASH_ATTR espFsFlags(EspFsFile *fh) {
	if (fh == NULL) {
		dbg_printf("File handle not ready\n");
		return -1;
	}
...
EspFsFile ICACHE_FLASH_ATTR *espFsOpen(char *fileName) {
	if (espFsData == NULL) {
		dbg_printf("Call espFsInit first!\n");
		return NULL;
	}
	char *p=espFsData;
	char *hpos;
	...
	while(1) {
		...
		if (h.magic!=ESPFS_MAGIC) {
			dbg_printf("Magic mismatch. EspFS image broken.\n");
			return NULL;
		}
		if (h.flags&FLAG_LASTFILE) {
			dbg_printf("End of image.\n");
			return NULL;
		}
...
//			dbg_printf("Alloc %p\n", r);
			...
			if (h.compression==COMPRESS_NONE) {
				...
				r->posComp++;
				dbg_printf("Heatshrink compressed file; decode parms = %x\n", parm);
				dec=heatshrink_decoder_alloc(16, (parm>>4)&0xf, parm&0xf);
				r->decompData=dec;
#endif
			} else {
				dbg_printf("Invalid compression: %d\n", h.compression);
				return NULL;
			}
			return r;
		}
...
//		dbg_printf("Reading %d bytes from %x\n", len, (unsigned int)fh->posComp);
		readFlashUnaligned(buff, fh->posComp, len);
		fh->posDecomp+=len;
		fh->posComp+=len;
//		dbg_printf("Done reading %d bytes, pos=%x\n", len, fh->posComp);
		return len;
#ifdef ESPFS_HEATSHRINK
...
//			dbg_printf("Elen %d rlen %d d %d pd %ld fdl %d\n",elen,rlen,decoded, fh->posDecomp, fdlen);

			if (elen == 0) {
				if (fh->posDecomp == fdlen) {
//					dbg_printf("Decoder finish\n");
