...
int ICACHE_FLASH_ATTR cgiUploadFirmware(HttpdConnData *connData) {
	...
	if (state==NULL) {
		...
		dbg_printf("Firmware upload cgi start.\n");
		state=malloc(sizeof(UploadState));
		if (state==NULL) {
			dbg_printf("Can't allocate firmware upload struct!\n");
			return HTTPD_CGI_DONE;
		}
		...
		if (state->state==FLST_START) {
			...
			if (def->type==CGIFLASH_TYPE_FW && memcmp(data, "EHUG", 4)==0) {
				...
				if (strcmp(buff, def->tagName)!=0) {
					dbg_printf("OTA tag mismatch! Current=`%s` uploaded=`%s`.\n",
										def->tagName, buff);
					len=httpdFindArg(connData->getArgs, "force", buff, sizeof(buff));
					if (len!=-1 && atoi(buff)) {
						dbg_printf("Forcing firmware flash.\n");
					} else {
				...
				state->state=FLST_ERROR;
				dbg_printf("Did not recognize flash image type!\n");
			}
			...
			if (dataLen<lenLeft) {
				...
				//Write page
				//dbg_printf("Writing %d bytes of data to SPI pos 0x%x...\n", state->pagePos, state->address);
	...
	if (connData->post->len==connData->post->received) {
		...
		dbg_printf("Upload done. Sending response.\n");
