...
void ICACHE_FLASH_ATTR wifiScanDoneCb(void *arg, STATUS status) {
	...
	cgiWifiAps.noAps=n;
	dbg_printf("Scan done: found %d APs\n", n);
	...
	while (bss_link != NULL) {
		if (n>=cgiWifiAps.noAps) {
			...
			dbg_printf("Huh? I have more than the allocated %d aps!\n", cgiWifiAps.noAps);
			break;
		}
		//Save the ap data.
...
static void ICACHE_FLASH_ATTR resetTimerCb(void *arg) {
	...
	if (x==STATION_GOT_IP) {
		...
		dbg_printf("Got IP. Going into STA mode..\n");
		wifi_set_opmode(1);
		system_restart();
	} else {
		connTryStatus=CONNTRY_FAIL;
		dbg_printf("Connect fail. Not going into STA-only mode.\n");
...
static void ICACHE_FLASH_ATTR reassTimerCb(void *arg) {
	int x;
	dbg_printf("Try to connect to AP....\n");
...
int ICACHE_FLASH_ATTR cgiWiFiConnect(HttpdConnData *connData) {
	...
	dbg_printf("Try to connect to AP %s pw %s\n", essid, passwd);
